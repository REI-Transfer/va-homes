      ...
      >
        <div className="flex items-center justify-between transition-all duration-300 px-4 pl-6 py-2.5">
          <Link href="#hero" className="shrink-0">
            <img src={config.logoUrl} alt={config.companyName} width={320} height={80} className="h-12 md:h-20 w-auto" />
          </Link>
            ...
            <div className="hidden md:flex flex-1 max-w-md mx-4 animate-reveal-up" style={{ animationDuration: '0.3s' }}>
              <div className="relative w-full flex items-center gap-2">
                ...
                <button
                  ...
                  className="shrink-0 h-9 px-4 bg-[var(--accent-secondary)] hover:bg-[var(--accent-secondary)/90] text-white text-sm font-medium rounded-lg transition-colors flex items-center gap-1.5"
                >
                  Go
          ...
          {pastHero && !showSurvey && (
            <button
              ...
              className="md:hidden px-4 py-1.5 text-xs font-medium rounded-full bg-[var(--accent-secondary)] text-white"
            >
              Get Offer
            </button>
          )}
          ...
          {!pastHero && (
            <button
              onClick={() => setShowSurvey(true)}
              className="px-5 py-2 text-sm font-medium transition-all rounded-full bg-[var(--accent-secondary)] text-white hover:bg-[var(--accent-brand)/90]"
            >
              Get Cash Offer
            </button>
          )}
